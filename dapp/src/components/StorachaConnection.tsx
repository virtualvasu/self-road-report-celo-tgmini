        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-green-900 mb-2">Connected to Your Storacha Account</h3>
              <div className="space-y-2">
                ...
                <div className="bg-white rounded-lg p-3 border border-green-200">
                  ...
                  <p className="text-sm text-gray-900 mt-1 font-mono break-all">{credentials.spaceDID}</p>
                </div>
              </div>
              <button onClick={() => onConnect({ email: '', spaceDID: '' })} className="mt-4 text-sm text-green-700 hover:text-green-800 underline">Change Account</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="mobile-wrapper">
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm mb-6">
        <div className="mb-6 bg-slate-50 rounded-md p-4">
          ...
          <p className="text-sm text-slate-600">Enter your Storacha credentials to upload the PDF to IPFS via your Web3.Storage account</p>
        </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ol className="list-decimal list-inside space-y-1 text-xs">
                <li>
                  Sign up at{' '}
                  <a href="https://console.web3.storage/" target="_blank" rel="noopener noreferrer" className="underline hover:no-underline">
        ...
        <div className="space-y-4">
          {/* Email Input */}
          <div>
            <label htmlFor="storacha-email" className="block text-sm font-medium text-gray-700 mb-2">Storacha Email Address</label>
            <input
              id="storacha-email"
              type="email"
              value={email}
          ...
          {/* Space DID Input */}
          <div>
            <label htmlFor="storacha-space-did" className="block text-sm font-medium text-gray-700 mb-2">Space DID</label>
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm ${errors.spaceDID ? 'border-red-300' : 'border-gray-300'}`}
            />
            {errors.spaceDID && (
              <div className="mt-1 flex items-center space-x-1 text-red-600">
                <AlertCircle className="w-4 h-4" />
          ...
          <div>
            <button
              ...
              className={`w-full py-3 px-4 rounded-md font-medium text-sm transition-all duration-200 flex items-center justify-center space-x-2 ${isValidating || !email || !spaceDID ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow-sm'}`}
            >
              {isValidating ? (
                <>
